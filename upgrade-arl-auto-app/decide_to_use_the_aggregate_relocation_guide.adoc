...
* When you are upgrading a four-node MetroCluster FC configuration, you select the `-nso false` option when issuing the CLI command.
+
Beginning with ONTAP 9.10.1, the automated negotiated switchover (NSO) based controller upgrade is the default procedure for a four-node MetroCluster FC configuration. If you do not select the `-nso false` option, the automated NSO based procedure is initiated.
