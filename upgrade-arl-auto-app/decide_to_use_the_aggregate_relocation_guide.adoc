...
* If you are upgrading controllers in a four-node MetroCluster FC configuration, you select the `false` for the `-nso` parameter when issuing the CLI command.
+
Beginning with ONTAP 9.10.1, the automated negotiated switchover (NSO) based upgrade is the default procedure for controllers in a four-node MetroCluster FC configuration. If you do not select the `-nso false` option, the automated NSO based procedure is initiated.
