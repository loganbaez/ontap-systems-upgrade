...
[NOTE]
====
* Beginning with ONTAP 9.10.1, the automated negotiated switchover (NSO) based upgrade procedure is the default for a four-node MetroCluster FC configuration. If you are upgrading a four-node MetroCluster FC configuration, to prevent the NSO based procedure being initiated, you must set the `-nso` parameter to `false` when you issue the `system controller replace start` command.
