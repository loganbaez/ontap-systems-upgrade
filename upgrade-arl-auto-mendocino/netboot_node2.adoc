---
sidebar: sidebar
permalink: upgrade-arl-auto-mendocino/netboot_node2.html
...
* You need to download the necessary system files for your system and the correct version of ONTAP from the _NetApp Support Site_. Refer to link:other_references.html[References] to link to the _NetApp Support Site_.
...
.	Wait for the node2 now running on the AFF A900 controller module to boot and display the boot menu options as shown below:
+
----
...
`halt`
+
Node2 should stop at LOADER> prompt.
...
+
`saveenv`
.	On node2, at the LOADER prompt, you should verify the `partner-sysid`. For node2, the `partner-sysid` needs to be that of node1. Verify the `partner-sysid` for node1:
+
